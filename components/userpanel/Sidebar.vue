<template>
    <!-- Overlay برای موبایل -->
    <div v-if="userSidebarStore.isMobile && userSidebarStore.isMobileOpen"
        @click="userSidebarStore.closeMobileSidebar()" class="fixed inset-0 bg-black bg-opacity-50 z-20 md:hidden">
    </div>

    <!-- سایدبار برای دسکتاپ -->
    <aside v-if="!userSidebarStore.isMobile" :class="[
        'bg-white shadow-lg transition-all duration-300 overflow-hidden fixed h-full z-20',
        userSidebarStore.isOpen ? 'w-64' : 'w-20'
    ]" style="height: calc(100vh - 4rem); top: 4rem;">
        <div class="p-4 flex flex-col h-full">
            <!-- دکمه باز و بسته کردن سایدبار -->
            <button @click="userSidebarStore.toggleSidebar()"
                class="self-end mb-6 p-2 rounded-full hover:bg-gray-100 transition-colors">
                <svg :class="{ 'rotate-180': !userSidebarStore.isOpen }"
            ...
            <nav class="flex-1">
                <ul class="space-y-2">
                    <li v-for="item in userSidebarStore.menuItems" :key="item.id">
                        <NuxtLink :to="item.path" @click="userSidebarStore.setActiveMenu(item.id)"
                            ...
                            <span :class="[
                                ...
                                userSidebarStore.isOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0 overflow-hidden'
                            ]">
                                {{ item.title }}
                            </span>
                        </NuxtLink>
    ...
    <aside v-if="userSidebarStore.isMobile" :class="[
        ...
        userSidebarStore.isMobileOpen ? 'translate-x-0' : '-translate-x-full'
    ]" style="width: 280px; height: 100vh;">
        <div class="p-4 flex flex-col h-full pt-20">
            <!-- دکمه بستن در موبایل -->
            <button @click="userSidebarStore.closeMobileSidebar()"
...
<script setup>
import { useUserSidebarStore } from '~/stores/usersidebar'

const userSidebarStore = useUserSidebarStore()
...
onMounted(() => {
    userSidebarStore.setActiveMenuFromRoute()
})
