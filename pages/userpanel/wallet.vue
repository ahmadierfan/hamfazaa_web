            ...
            <div class="p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-green-50 to-blue-50">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-gray-600">مجموع واریزی ها - ریال</p>
                        ...
                        <tr v-for="transaction in paginatedTransactions" :key="transaction.id"
                            ...
                            <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 sm:px-3 sm:py-1 inline-flex text-xs leading-5  rounded-full"
                                    :class="getStatusBadgeClass(transaction.ispayed)">
                                    {{ getStatusText(transaction.ispayed) }}
                                </span>
                            </td>
                        </tr>
                    </tbody>
                ...
                <div v-for="transaction in paginatedTransactions" :key="transaction.id"
                    ...
                    <div class="grid grid-cols-1 gap-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-900">{{ getStatusText(transaction.ispayed) }}</span>
                        </div>
...
const getStatusText = (ispayed) => {
    const texts = {
        1: 'تأیید شده',
        0: 'در انتظار'
    }
    return texts[ispayed] || ispayed
}
...
const getTransactions = async () => {
    try {
        const { data } = await $freeApi.get('user-wallet-transactions')
